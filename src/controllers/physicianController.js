...
function passwordValidation(password) {
  if(password.length < 8)
		...
		return "A Senha deve ter no mínimo uma letra minuscula.";
	else if(!password.match(/[A-Z]/g))
		return "A Senha deve ter no mínimo uma letra maíuscula.";
	else if(!password.match(/[@!#$%^&*()/\\]/g))
		return "A Senha deve ter no mínimo um caracter especial.";
...
module.exports = {
  ...
  async authentication(req, res) {
    ...
    if(!email || !password)
      return res.status(400).json({ msg: "Campos obrigatórios estão vazios!" });
    try{
  ...
  async newPhysician(req, res) {
    ...
    if (isPhysicianNew)
      ...
      if (physician)
        ...
        res.status(404).json({ msg: "Não foi possivel cadastrar novo medico" });
    }
  },
  ...
  async listAllPhysician(req, res) {
    ...
    if (physicians) res.status(200).json({ physicians });
      ...
      res.status(404).json({
        msg: "Não foi possível encontrar medicos",
      });
  },
